...
authRouter.post('/signup' , validation(signupSchema) , async (req: Request , res: Response)=>{
    let userData = await authService.signup(req.body)
    SuccessResponse({res , message: "signup success" , data: userData , status: 201})
})


authRouter.post('/verify_email' , async (req: Request , res: Response)=>{
...
authRouter.post('/forgetPassword' , auth , async (req:Request , res:Response)=>{
...
authRouter.patch('/resetPassword' , auth , async (req:AuthenticatedRequest , res: Response)=>{
...
authRouter.get('/get_all_users' , async (req: Request , res: Response)=>{
    let users = await authService.getAllUsers()
    SuccessResponse({res , data: users, message: "get all users succ", status: 200})
})

authRouter.get('/get_user_by_id'  ,auth,  async (req: AuthenticatedRequest , res: Response)=>{
...
authRouter.patch('/update_user_by_id' , auth , async (req: AuthenticatedRequest , res: Response)=>{
...
authRouter.delete('/delete_user_by_id' , auth , async (req: AuthenticatedRequest , res: Response)=>{
...
authRouter.post('/revokeToken' , auth , async (req: AuthenticatedRequest , res: Response )=>{
    let token = await authService.revokeToken(req.Token)
    SuccessResponse({res , message: "token revoked"  , data: token , status: 200})
})


authRouter.post('/view_profile' , auth,  async(req: AuthenticatedRequest , res: Response)=>{
