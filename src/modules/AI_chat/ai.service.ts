...
import fs from 'fs'
import { groq_api, HF_TOKEN, image_api } from '../../config/env.service'
            ...
            if (intent.type === "video") {
                ...
                const response = await fetch(
                    "https://api-inference.huggingface.co/models/black-forest-labs/FLUX.1-schnell",
                    {
                        ...
                        body: JSON.stringify({
                            inputs: intent.content || prompt,
                        }),
                    }
                )
                const imageBuffer = await response.arrayBuffer()
                const fileName = `image-${data}.png`
                await fs.promises.writeFile(`./public/images/${fileName}`, Buffer.from(imageBuffer))
                mediaUrl = `http://localhost:3000/public/images/${fileName}`
